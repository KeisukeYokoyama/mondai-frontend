...
function EditSpeakerContent({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchSpeakerData = async () => {
      try {
        ...
        if (data) {
          setFormData({
            speaker_type: data.speaker_type?.toString() || '',
            ...
            parent_party_id: data.party_id?.toString() || '',
            prefecture_id: data.prefecture_id?.toString() || '',
            district: data.district || '',
            chamber: data.chamber || '',
            election_result: data.election_result?.toString() || '',
          ...
          if (data.image_path) {
            const { data: imageData } = await getSupabaseClient()
              .storage
              .from('politicians')
              .getPublicUrl(data.image_path);
            setImagePreview(imageData.publicUrl);
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const speakerData = {
        ...
        image_path: image_path || formData.image_path,
      };
      ...
      showToastMessage('発言者を更新しました', 'success');
      setTimeout(() => {
        router.push('/commentators');
      }, 2000);

    } catch (error) {
      console.error('発言者の更新に失敗しました:', error);
      showToastMessage('発言者の更新に失敗しました');
    }
  };
    ...
    <main className="w-full max-w-full overflow-x-hidden bg-gray-100">
      ...
      {showUpdateConfirm && (
        <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-28 border-2">
          <div className="fixed inset-0 bg-gray-600/10 backdrop-blur-xl" onClick={() => setShowUpdateConfirm(false)}></div>
          <div className="relative bg-white rounded-lg p-6 max-w-screen-md w-full mx-4 z-10 shadow-xl/30">
            ...
            <p className="mb-4 text-sm">入力内容に問題はないですか？</p>
            <p className="mb-4 text-sm">内容を確認して更新してください。</p>
            <div className="flex justify-end space-x-3">
              <button
...
export default function EditSpeaker({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense fallback={<div>読み込み中...</div>}>
      <EditSpeakerContent params={params} />
    </Suspense>
  );
}
